...
async def mine_repo_rf_activity(result_dir: Path, logs_dir: Path, project_repo: Repo) -> Path:
...
async def mine_refactoring_activity(project_repos: list[Repo]) -> list[Path]:
    ...
    tasks = [mine_repo_rf_activity(result_dir, logs_dir, project_repo) for project_repo in project_repos]
    results: list[Path] = await asyncio.gather(*tasks)
    return results


async def get_commit_diff_data_from_repo(diff_result_dir: Path, repo: Repo) -> bool:
...
async def mine_diffs(project_repos: list[Repo]) -> bool:
    ...
    tasks = [get_commit_diff_data_from_repo(diff_result_dir, repo) for repo in project_repos]
...
async def mine_effort_for_repo(tloc_result_dir: Path, repo: Repo) -> bool:
...
async def mine_effort(project_repos: list[Repo]) -> bool:
    ...
    tasks = [mine_effort_for_repo(tloc_result_dir, repo) for repo in project_repos]
    await asyncio.gather(*tasks)
    return True
